...
With the SHA in hand, look up the linked agent session via the `memory_commit_lookup` MCP tool with `sha: "<full-sha>"`.

**Note:** `memory_commit_lookup` is only available in **Proxy mode** (REST API online). In Local mode, this tool won't exist. Verify mode first with `tool_search`. If unavailable, fall back to HTTP: `GET $AGENTMEMORY_URL/agentmemory/session/by-commit?sha=<sha>` with `Authorization: Bearer $AGENTMEMORY_SECRET` when the secret is set.
